...
Api.prototype.signup = function signup(options) {
    ...
    server = new JenkinsServer(options.encoding, options.url, fileSystem);
...
//Exact match with a regular expression
...
Api.prototype.status = function status(options, cb) {
    ...
    var windowsFullPath = /\w\:\\.+/
    try {
    	if (connection.startsWith("/") || matchExact(windowsFullPath, connection)) {
    		jenkins = require(connection);
    	}
    	else {
    		// Assume relative path in any other case
    		jenkins = require(process.cwd() + "/" + connection);
    	}
        fileSystem = new FileSystem(jenkins.pipelineDirectory);
        server = new JenkinsServer(jenkins.encoding, jenkins.url, fileSystem,jenkins.auth);
