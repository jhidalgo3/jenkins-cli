...
// This function tries to obtain a breacrumb from Jenkins
// If this functionality is not enabled in Jenkins, it just
//	returns silently a null value
function getJenkinsCrumb(encoding, serverUrl, fileSystem, auth) {
	var crumb = '';
	try {
		var options = {
		...
		crumb = jsonObj.defaultCrumbIssuer.crumb;
	}
	catch(e) {
		crumb = null;
	}
...
function JenkinsServer(encoding, serverUrl, fileSystem, auth) {
    ...
    this.fetchJobs = function (filter) {
        ...
        var options = {
            ...
            method: 'POST',
            headers: {
            	'Jenkins-Crumb': getJenkinsCrumb(encoding, serverUrl, fileSystem, auth),
    ...
    this.fetchJobConfigurations = function (jobs) {
        ...
        var crumb = getJenkinsCrumb(encoding, serverUrl, fileSystem, auth);
        var deferred = q.defer();
    ...
    function fetchJobConfigurationStrategy(job) {
        var deferred = q.defer();
        var urlConfig = [serverUrl, 'job', urlencode(job), 'config.xml'].join('/');
